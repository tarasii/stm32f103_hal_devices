...
HAL_StatusTypeDef BMP180_Init(BMP180_t* BMP180_Data) {
	uint8_t data[BMP180_EEPROM_SIZE];
	uint8_t i = 0;
	...
	I2C_ReadBuf(&hi2c1, BMP180_I2C_ADDRESS, BMP180_REGISTER_EEPROM, data, BMP180_EEPROM_SIZE);
...
HAL_StatusTypeDef BMP180_StartTemperature(BMP180_t* BMP180_Data) {
	uint8_t data = BMP180_COMMAND_TEMPERATURE;
	...
	/* Send to device */
	I2C_WriteBuf(&hi2c1, BMP180_I2C_ADDRESS, BMP180_REGISTER_CONTROL, &data, 1);
...
HAL_StatusTypeDef BMP180_ReadTemperature(BMP180_t* BMP180_Data) {
	...
	I2C_ReadBuf(&hi2c1, BMP180_I2C_ADDRESS, BMP180_REGISTER_RESULT, data, 2);
...
HAL_StatusTypeDef BMP180_StartPressure(BMP180_t* BMP180_Data, BMP180_Oversampling_t Oversampling) {
	...
	/* Send to device */
	I2C_WriteBuf(&hi2c1, BMP180_I2C_ADDRESS, BMP180_REGISTER_CONTROL, &command, 1);
...
HAL_StatusTypeDef BMP180_ReadPressure(BMP180_t* BMP180_Data) {
	...
	/* Read multi bytes from I2C */
	I2C_ReadBuf(&hi2c1, BMP180_I2C_ADDRESS, BMP180_REGISTER_RESULT, data, 3);
...
float BMP180_GetHgPressure(uint32_t pressure) {
	//return (float)pressure * 0.0075;
	//return (float)pressure * 3 / 400;	
	return (float)pressure * 750.061683 / 100000.0;
